...
filepath = "/home/pi/Desktop/projectCam/project"
...
def saveImage(settings, width, height, quality, diskSpaceToReserve):
    ...
    imageName = filenamePrefix + "-%04d%02d%02d-%02d%02d%02d.jpg" % (time.year, time.month, time.day, time.hour, time.minute, time.second)
    subprocess.call("raspistill %s -w %s -h %s -t 200 -e jpg -q %s -n -o %s" % (settings, width, height, quality, filename), shell=True)
    print ("Captured %s" % filename)
    print ("Image Name: %s" % imageName)
    print ("Sending to email: %s" % imageName)
    print ("Morse Code")
    morsecode()
    capture_Email(imageName)
...
#send mail
def capture_Email(imgName):
    #import motionCam.py
    #import buzzer
    
    img_file = imgName
    recipient = "nanawan505@gmail.com"
    ...
    if username and password and img:
        ...
        text = MIMEText("Intruder Detected")
        msg.attach(text)
